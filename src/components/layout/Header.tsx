import { BookOpen, Plus, RefreshCw, Settings, LayoutGrid, Table2, Sun, Moon, Wand2 } from 'lucide-react'
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 h-12 sm:h-14 flex items-center gap-2">
        ...
        <div className="flex items-center bg-surface2 border border-border rounded-lg p-0.5 gap-0.5">
          {(['card', 'table'] as const).map((mode) => {
            const Icon = mode === 'card' ? LayoutGrid : Table2
            return (
              <button
                key={mode}
        ...
        <button
          ...
          className="flex items-center gap-1.5 h-8 px-2.5 sm:px-3 rounded-lg bg-brand text-white text-xs font-medium hover:opacity-90 transition-opacity shadow-sm shrink-0"
        >
